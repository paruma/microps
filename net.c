...
static struct net_device *devices; // BSS領域にある。デバイスのlinked listの先頭を表すポインタ
...
{
    struct net_device *dev;
    dev = memory_alloc(sizeof(*dev)); // 構造体をヒープ領域に確保するにはこうやる（内部でcallocが呼ばれている）
    if (!dev) {
...
net_device_output(struct net_device *dev, uint16_t type, const uint8_t *data, size_t len, const void *dst)
{
    ...
    debugdump(data, len);
    // dev->ops>transmitは非NULL (openやclosedはNULL許容)
