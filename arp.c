...
// cachesの中から使っていないエントリを探して返す。使っていないエントリがない場合は一番古いエントリの中身を削除して返す。
...
// cacheの中から、Protocol Address が pa のものを返す（ない場合はNULLを返す）
static struct arp_cache *
...
// Protocol Address が pa である cache の情報(Hardware Address)を更新する
static struct arp_cache *
...
arp_input(const uint8_t *data, size_t len, struct net_device *dev)
{
    ...
    // ハードウェアアドレスのチェック
    bool hardware_addr_is_ether = ntoh16(msg->hdr.hrd) == ARP_HRD_ETHER && msg->hdr.hln == ETHER_ADDR_LEN;
    if (!hardware_addr_is_ether) {
        errorf("unsupported hardware address");
        return;
    }

    // プロトコルアドレスのチェック
    bool protocol_addr_is_ip = ntoh16(msg->hdr.pro) == ARP_PRO_IP && msg->hdr.pln == IP_ADDR_LEN;
    if (!protocol_addr_is_ip) {
...
arp_timer_handler(void)
{
    ...
    for (entry = caches; entry < tailof(caches); entry++) {
        if (entry->state != ARP_CACHE_STATE_FREE && entry->state != ARP_CACHE_STATE_STATIC) {
            ...
            if (diff.tv_sec > ARP_CACHE_TIMEOUT) { /* true (!0) or false (0) */
