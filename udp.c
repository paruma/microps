...
udp_input(const uint8_t *data, size_t len, ip_addr_t src, ip_addr_t dst, struct ip_iface *iface)
{
    ...
    entry = memory_alloc(sizeof(*entry) + hdr->len);
    if (!entry) {
    ...
    entry->len = hdr->len;
    memcpy(entry->data, (uint8_t *)(hdr + 1), hdr->len);
...
udp_open(void)
{
    ...
    pcd = udp_pcb_alloc();
    if (pcd == NULL) {
        errorf("udp_pcd_alloc() failure");
        return -1;
    }
    return udp_pcb_id(pcd);
}
...
udp_bind(int id, struct ip_endpoint *local)
{
    ...
    // エンドポイント local が使われていないか確認する
    exist = udp_pcb_select(local->addr, local->port);
    if (exist != NULL) {
        errorf("%s is already used.", ip_endpoint_ntop(&pcb->local, ep2, sizeof(ep2)));
        return -1;
    }
